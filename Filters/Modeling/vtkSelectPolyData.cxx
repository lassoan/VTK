  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  vtkIdType cellIdInSelectedRegion = this->ComputeTopologicalDistance(mesh, edgePointIds, pointMarks, cellMarks);
...
//------------------------------------------------------------------------------
bool vtkSelectPolyData::IsBoundaryEdge(vtkIdType pointId1, vtkIdType pointId2, vtkIdList* edgePointIds)
{
  vtkIdType numMeshLoopPts = edgePointIds->GetNumberOfIds();
  for (vtkIdType edgePointIndex = 0; edgePointIndex < numMeshLoopPts; edgePointIndex++)
  {
    ...
    {
      vtkIdType edgePointIdAfter =
        edgePointIds->GetId((edgePointIndex + 1) % numMeshLoopPts);
      if (edgePointIdAfter == pointId1)
      {
        // boundary edge
        return true;
      }
      vtkIdType edgePointIdBefore =
        edgePointIds->GetId((edgePointIndex - 1) % numMeshLoopPts);
...
void vtkSelectPolyData::SetSelectionScalarsToOutput(vtkPointData* originalPointData,
  vtkCellData* originalCellData, vtkPolyData* mesh, vtkIdList* edgePointIds, vtkIntArray* pointMarks, vtkPolyData* output)
{
  vtkPoints* inPts = mesh->GetPoints();
  vtkIdType numPts = inPts->GetNumberOfPoints();
  ...
  for (vtkIdType edgePointIndex = 0; edgePointIndex < numMeshLoopPts; edgePointIndex++)
  {
    ...
    double closestPointOnBoundaryPos[3] = { 0.0, 0.0, 0.0 }; // closest position of edgePoint on the boundary
    ...
    // First compute distance assuming that x is on the same side of the boundary as the farthest neighbor.
    double dist = sqrt(closestDist2);
    if (pointMarks->GetValue(farthestNeighborPointId) < 0)
    {
      dist = -dist;
    }
    // If x and the farthest neighbor are actually different sides of the boundary then invert the signed distance value.
    double farthestNeighborPointPos[3];
    inPts->GetPoint(farthestNeighborPointId, farthestNeighborPointPos);
    if (vtkMath::Distance2BetweenPoints(farthestNeighborPointPos, x) > vtkMath::Distance2BetweenPoints(farthestNeighborPointPos, closestPointOnBoundaryPos))
    {
